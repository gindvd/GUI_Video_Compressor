...
class FrameViewer(ctk.CTkToplevel):
    ...
    def __init__(
        self, master: Any, ffmpeg_handler: FFmpegProcessHandler, **kwargs: Any
    ) -> None:
    ...
    def _build_ui(self) -> None:
        ...
        # Image canvas
        self._canvas = tk.Canvas(self, bg="black", highlightthickness=0, borderwidth=0)

        self._canvas.pack(padx=10, pady=(10, 5), fill="both", expand=True)

        # Resize the fame image when the window is resized
        self._canvas.bind("<Configure>", self._on_canvas_resize)

        self._image_id: int | None = None
        self._text_id: int | None = self._canvas.create_text(
            0, 0, text="No media loaded", fill="white", anchor="center"
        )

        # Control frame: controls for fingding frames
        control_frame = ctk.CTkFrame(
            self, fg_color=("gray75", "gray25"), corner_radius=6
        )
        ...
        self._save_button = ctk.CTkButton(
            info_frame, text="Save", command=self._save_frame
        )
    ...
    def load_media(self, file_path: str, duration_s: float, fps_str: str) -> None:
        ...
        self._slider.configure(
            to=self._duration_ms, number_of_steps=steps, state="normal"
        )
    ...
    def _extract_and_display(self, ms: float) -> None:
        ...
        extracted, frame_bytes = self._ffmpeg_handler.extract_frame(
            self._file_path, timestamp
        )
    ...
    def _display_image(self) -> None:
        ...
        resized = self._img.resize((new_width, new_height), Image.Resampling.LANCZOS)
    ...
    def _update_info_labels(self) -> None:
        ...
        self._current_frame = (
            int(self._current_ms / self._frame_duration_ms)
            if self._frame_duration_ms > 0
            else 0
        )
        total_frames = (
            int(self._duration_ms / self._frame_duration_ms)
            if self._frame_duration_ms > 0
            else 0
        )
